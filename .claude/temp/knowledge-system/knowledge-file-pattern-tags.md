...
Edit `.claude/kv/scripts/knowledge-lib.mjs`:
...
Edit `.claude/kv/scripts/validate-knowledge.mjs`:
...
jq '.knowledge."frontend/core/components"."component-implementation-patterns".tags += ["tsx", "jsx"]' \
  .claude/kv/knowledge.json > tmp.json && mv tmp.json .claude/kv/knowledge.json
```
