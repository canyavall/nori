...
**Script**: `.claude/kv/scripts/build-knowledge-index.mjs`

**Features**:
- Scans all `.claude/kv/vault/**/*.md` files
...
**Implementation**: `.claude/kv/scripts/lib/frontmatter.mjs`
...
- Handles patterns used in knowledge vault:
  ...
  - Simple values: `description: My package`
...
**Cache file**: `.claude/kv/.cache/parsed-metadata.json`
...
**Hook**: `.claude/kv/hooks/session-start-cleanup.mjs`
...
```bash
# Backup original
cp .claude/kv/knowledge.json .claude/kv/knowledge.json.backup

# Migrate to frontmatter
node .claude/kv/scripts/migrate-to-frontmatter.mjs

# Build from frontmatter
node .claude/kv/scripts/build-knowledge-index.mjs
...
**Script**: `.claude/kv/scripts/validate-frontmatter.mjs`

**Checks**:
- Required fields present (tags, description, required_knowledge, rules)
...
**Usage**:
```bash
node .claude/kv/scripts/validate-frontmatter.mjs
```
...
node .claude/kv/scripts/build-knowledge-index.mjs --force-rebuild
```
