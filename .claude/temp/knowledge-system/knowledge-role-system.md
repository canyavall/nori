...
### User Settings (`.claude/kv/settings.json`)
...
**File**: `.claude/kv/sessions/session-state.json` (gitignored)
...
cp .claude/kv/settings.json.example .claude/kv/settings.json
...
**Usage**:
```bash
node .claude/kv/scripts/session-manager.mjs init
node .claude/kv/scripts/session-manager.mjs add pkg1,pkg2
node .claude/kv/scripts/session-manager.mjs list
```
...
node .claude/kv/scripts/load-role.mjs

# Override role
node .claude/kv/scripts/load-role.mjs --role qa
```
...
**Location**: `.claude/kv/vault/shared/`
...
**File**: `.claude/kv/hooks/session-start-cleanup.mjs`
