...
tags:
  ...
  - kv-search-cli
  - kv-tag-search
description: 'Core knowledge search usage, flags, and output formats'
required_knowledge: []
---
...
**Search by tags:**
```bash
node .claude/kv/scripts/knowledge-search.mjs --tags routing,react-router
```
Returns all packages matching ANY tags (OR logic).

**Search by category:**
```bash
node .claude/kv/scripts/knowledge-search.mjs \
  --tags testing,jest \
...
**List all categories:**
```bash
node .claude/kv/scripts/knowledge-search.mjs --list-categories
```

**List all tags:**
```bash
node .claude/kv/scripts/knowledge-search.mjs --list-tags
```

**List tags by category:**
```bash
node .claude/kv/scripts/knowledge-search.mjs --list-categories | grep "frontend/testing"
```
...
```json
{
  ...
  "results": [{
    ...
    "knowledge_path": ".claude/kv/vault/frontend/routing/react-router-basics.md"
  }]
}
```
