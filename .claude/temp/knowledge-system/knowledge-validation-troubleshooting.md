...
tags:
  - kv-knowledge
  - kv-validation
  - kv-validation-troubleshooting
description: Common issues and best practices for knowledge validation
required_knowledge: []
---
...
**Error:**
```
⚠️  WARNING: Orphaned file (not in knowledge.json): .claude/kv/vault/frontend/testing/legacy-test.md
```
...
**Error:**
```
❌ ERROR: File not found: .claude/kv/vault/frontend/routing/react-router-basics.md
```
...
```json
{
  ...
  "knowledge_path": ".claude/kv/vault/frontend/testing/testing-core.md"
}
```
...
```bash
# Before committing
node .claude/kv/scripts/validate-knowledge.mjs

# After reorganization
node .claude/kv/scripts/validate-knowledge.mjs --quick

# Weekly check for orphans
node .claude/kv/scripts/validate-knowledge.mjs | grep "Orphaned"
```
