...
export function KnowledgeBrowser() {
  ...
  const handleVaultSwitchConfirm = async () => {
    ...
    try {
      ...
      // Get workspace by path to get ID
      const workspace = await invoke<Workspace | null>('get_workspace_by_path', { path: workspacePath });
      if (!workspace) {
        throw new Error('Workspace not found');
      }

      // Update workspace vault via Tauri command
      await invoke('update_workspace_vault', {
      ...
      // Update all tabs using this workspace
      const affectedTabs = tabs.filter(tab => tab.workspace?.path === workspacePath);
      for (const tab of affectedTabs) {
        // Fetch updated workspace data
        const updatedWorkspace = await invoke<Workspace | null>('get_workspace_by_path', { path: workspacePath });
