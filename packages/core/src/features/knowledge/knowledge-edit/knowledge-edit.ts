...
export async function runKnowledgeEdit(
  ...
  const validateResult = validateChanges(input.title, input.category, input.tags, input.description, input.required_knowledge, input.rules, input.content);
  ...
  // Step 05: Regenerate index — update existing DB entry (non-fatal)
